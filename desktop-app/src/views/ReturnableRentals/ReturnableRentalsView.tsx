...
import {ReturnableRentalsViewState} from "./ReturnableRentalsViewState";
import {ReturnableRentalsViewProps} from "./ReturnableRentalsViewProps";
...
function ReturnableRentalsView(props:ReturnableRentalsViewProps):JSX.Element {

    const [state, setState] = useState<ReturnableRentalsViewState>({
        offset: 0,
        text: "",
    ...
    function renderItem(rental: Rental, key: number){
        ...
        return(
            <tr className={styles.rental} key={key}>
                ...
                <td>{returnDate}</td>
                <td>{HoursFormatter.hoursToString(rental.remainingTime)}</td>
                <td>{rentDate}</td>
            </tr>);
    }
...
export default ReturnableRentalsView;
